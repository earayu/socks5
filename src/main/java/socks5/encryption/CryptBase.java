...
public abstract class CryptBase implements ICrypt {
	...
	@Override
	public ByteBuf encrypt(ByteBuf byteBuf){
		int len = byteBuf.readableBytes();
		byte[] arr = new byte[len];
		byteBuf.getBytes(0, arr);
		ByteArrayOutputStream _remoteOutStream = new ByteArrayOutputStream();
		encrypt(arr, len, _remoteOutStream);
		return Unpooled.wrappedBuffer(_remoteOutStream.toByteArray());
	}

	@Override
	public ByteBuf decrypt(ByteBuf byteBuf){
		int len = byteBuf.readableBytes();
		byte[] arr = new byte[len];
		byteBuf.getBytes(0, arr);
		ByteArrayOutputStream _localOutStream = new ByteArrayOutputStream();
		decrypt(arr, len, _localOutStream);
		return Unpooled.wrappedBuffer(_localOutStream.toByteArray());
	}
